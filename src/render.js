...
function drawNoticeFooter(ctx, model, footer, colors) {
  ...
  const titleH = 34; ctx.fillStyle = green; ctx.fillRect(x, y, w, titleH); ctx.fillStyle = '#fff'; ctx.font = '700 20px Arial'; ctx.textAlign = 'center'; ctx.fillText(`AVISO DE DESMONTE - Nº ${model.meta.blastNumber || '____'}/${noticeYear(model.meta)}`, x + w / 2, y + 23);
  const logoW = 315; const infoX = x + logoW; const infoH = 100; ctx.strokeStyle = border; ctx.strokeRect(x, y + titleH, logoW, infoH); ctx.strokeRect(infoX, y + titleH, w - logoW, infoH);
  if (model.logoImage) { const ratio = Math.min((logoW - 42) / model.logoImage.width, (infoH - 20) / model.logoImage.height); const lw = model.logoImage.width * ratio; const lh = model.logoImage.height * ratio; ctx.drawImage(model.logoImage, x + (logoW - lw) / 2, y + titleH + (infoH - lh) / 2, lw, lh); }
  const dateText = model.meta.date ? model.meta.dateLabel : 'Não informada'; const timeText = model.meta.time || 'Não informado'; ctx.textAlign = 'left'; ctx.fillStyle = colors.ink; ctx.font = '700 18px Arial'; ctx.fillText('DATA:', infoX + 10, y + titleH + 25); ctx.font = '18px Arial'; ctx.fillText(dateText, infoX + 78, y + titleH + 25); ctx.font = '700 18px Arial'; ctx.fillText('HORÁRIO:', infoX + 10, y + titleH + 53); ctx.font = '18px Arial'; ctx.fillText(timeText, infoX + 116, y + titleH + 53); ctx.font = '700 18px Arial'; ctx.fillText('REGIÕES DE DESMONTE DE ROCHA:', infoX + 10, y + titleH + 82); ctx.font = '18px Arial'; ctx.fillText(fitCanvasText(ctx, noticeRegions(model) || 'Não informadas', w - logoW - 365), infoX + 350, y + titleH + 82);
